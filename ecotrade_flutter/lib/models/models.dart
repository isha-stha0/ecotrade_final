...
class UserModel {
  ...
  factory UserModel.fromJson(Map<String, dynamic> j) => UserModel(
    id: j['_id'] ?? j['id'] ?? '', name: j['name'] ?? j['full_name'] ?? '', email: j['email'] ?? '',
    role: j['role'] ?? 'customer', phone: j['phone'], address: j['address'],
    ecoPoints: j['ecoPoints'] ?? j['reward_points'] ?? 0, totalScraps: (j['totalScraps'] ?? 0).toDouble(),
...
class ScrapModel {
  ...
  ScrapModel({required this.id, required this.category, required this.description,
    ...
    this.location, this.adminNotes, this.collectorId, this.pointsAwarded = 0,
    required this.createdAt, this.completedAt, this.user, this.collector});
  ...
  factory ScrapModel.fromJson(Map<String, dynamic> j) {
    ...
    final collectorMap = collectorValue is Map ? Map<String, dynamic>.from(collectorValue) : null;
    return ScrapModel(
      id: j['_id'] ?? '', category: j['category'] ?? '',
      description: j['description'] ?? '', quantity: (j['quantity'] ?? j['quantity_estimated'] ?? 0).toDouble(),
      unit: j['unit'] ?? 'kg', status: j['status'] ?? 'pending',
      pickupLat: pickup is Map && pickup['lat'] != null ? (pickup['lat'] as num).toDouble() : null,
      pickupLng: pickup is Map && pickup['lng'] != null ? (pickup['lng'] as num).toDouble() : null,
      location: j['location'] ?? j['pickup_address'], adminNotes: j['adminNotes'] ?? j['admin_notes'],
      collectorId: collectorMap?['_id']?.toString() ?? collectorMap?['id']?.toString() ?? (collectorValue is String ? collectorValue : null),
      pointsAwarded: j['pointsAwarded'] ?? j['points_awarded'] ?? 0,
      createdAt: DateTime.tryParse(j['createdAt'] ?? '') ?? DateTime.now(),
      completedAt: DateTime.tryParse(j['completed_at'] ?? j['completedAt'] ?? ''),
      user: j['user'] is Map ? j['user'] : (j['user_id'] is Map ? j['user_id'] : null),
...
class ProductModel {
  ...
  ProductModel({required this.id, required this.name, required this.description,
    ...
    this.sold = 0, this.madeFrom, this.ecoImpact, this.sourceMaterial,
    this.status, this.imageUrls = const [], this.isActive = true});
  ...
  factory ProductModel.fromJson(Map<String, dynamic> j) {
    ...
    return ProductModel(
      ...
      stock: (j['stock'] ?? j['stock_quantity'] ?? 0) is num
          ...
          : int.tryParse((j['stock'] ?? j['stock_quantity'] ?? '0').toString()) ?? 0,
      sold: (j['sold'] ?? 0) is num ? (j['sold'] as num).toInt() : int.tryParse('${j['sold']}') ?? 0,
      madeFrom: j['madeFrom'],
